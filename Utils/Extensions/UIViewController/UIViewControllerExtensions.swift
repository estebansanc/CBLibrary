    ...
    func showSkeleton() {
        DispatchQueue.main.async {
            ...
            let animation = SkeletonAnimationBuilder()
                .makeSlidingAnimation(withDirection: .bottomRightTopLeft,
                                      duration: 1,
                                      autoreverses: true)

            let baseColor = UIColor.init(white: 0.95, alpha: 1)
            let secondaryColor = UIColor.white
            let skeletonGradient = SkeletonGradient(baseColor: baseColor,
                                                    secondaryColor: secondaryColor)
            
            self.view.showAnimatedGradientSkeleton(usingGradient: skeletonGradient,
                                                   animation: animation,
